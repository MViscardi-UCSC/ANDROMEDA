...
def load_single_reference_contig(reference_path: str | Path, contig: str = None):
    ...
    if len(records) > 1:
        if contig is None:
            log.info(
                f"Found {len(records)} records in reference file, "
            ...
            raise ValueError(
                f"Contig {contig} not found in reference file, "
        ...
        if contig is not None and contig not in record_ids:
            log.info(
                f"Found 1 record in reference file ({record_ids[0]}), "
            ...
            log.info(
                f"Found 1 record in reference file (matching your request for {contig}), using it."
            )
        target_record = records[0]
        target_contig = record_ids[0]
...
def load_reference_contigs_to_dict(reference_path: str | Path) -> Dict[str, str]:
    ...
    log.debug(
        f"Loaded {len(records)} contigs ({', '.join(records_dict.keys())}) "
