...
def run_umi_tools_group(
    ...
    assert output_parent_dir.exists(), (
        f"Output directory does not exist: {output_parent_dir}"
    )
    ...
    if edit_frac <= 0 and edit_dist <= 0:
        ...
        log.info(
            "No edit distance or fraction provided, defaulting to 0 (perfect match only)."
        )
        edit_dist = 0
        ...
        log.info(
            f"Calculated edit distance: {edit_dist} from fraction: {edit_frac}"
            f" and UMI length: {len(umi)}"
        )
    elif edit_frac > 0 and edit_dist > 0:
        log.info(
            f"Both edit distance and fraction provided, using provided edit distance of {edit_dist}."
        )

    # Output files
    grouped_bam = (
        output_dir / tagged_bam.with_suffix(f".grouped_{edit_dist}dist.bam").name
    )
    group_out_tsv = (
        output_dir / tagged_bam.with_suffix(f".grouped_{edit_dist}dist.tsv").name
    )
    group_log = (
        output_dir / tagged_bam.with_suffix(f".grouped_{edit_dist}dist.log").name
    )
    ...
    group_call = [
        ...
        str(group_out_tsv),
        "--method",
        "directional",  # This seemed to work well (and is default) but we could try others!
    ...
    if per_gene and not per_contig:
        ...
        log.info(
            "🔍 --per-contig grouping option selected, adding --per-gene (umi-tools requires this)."
        )
        group_call.extend(["--per-gene", "--per-contig"])
    else:
    ...
    log.warning(
        f"Starting command, this will not produce any outputs until complete, please be patient!"
    )
    process = subprocess.Popen(
        group_call, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Group UMIs using `umi_tools group` with a single edit distance."
    )

    parser.add_argument(
        "tagged_bam", type=Path, help="Path to input BAM file (must have UMIs tagged)."
    )
    parser.add_argument(
        "output_parent_dir",
        type=Path,
    ...
    parser.add_argument("--per-gene", action="store_true", help="Group per gene.")
    parser.add_argument(
        "--per-cell",
        action="store_true",
    ...
    parser.add_argument(
        "--grouping-plot", action="store_true", help="Plot UMI group size distribution."
    )
