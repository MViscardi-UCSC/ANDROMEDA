...
class NucleotideQuality:
    ...
    def __init__(self, char: str = None, q_score: int = None, prob_error: float = None):
        ...
        if not any(
            ...
            raise ValueError(
                "At least one of the following must be set: char, q_score, prob_error."
            )

    def __add__(self, other):
        if isinstance(other, NucleotideQuality):
            return NucleotideQuality(
                prob_error=(self.to_prob_error() + other.to_prob_error())
            )
        elif isinstance(other, (int, float)):
            return NucleotideQuality(prob_error=(self.to_prob_error() + other))
        else:
            raise TypeError(
                f"Unsupported type for addition with NucleotideQuality class: {type(other)}"
            )

    def __mul__(self, other):
        if isinstance(other, NucleotideQuality):
            raise NotImplementedError(
                "Multiplication of two NucleotideQuality objects is not supported "
            ...
            raise TypeError(
                f"Unsupported type for multiplication with NucleotideQuality class: {type(other)}"
            )
    ...
    def to_phred_char(self) -> str:
        if self.char is not None:
            ...
            raise ValueError(
                "No Phred score information set. This should be unreachable?!"
            )
    ...
    def to_q_score(self) -> int:
        if self.q_score is not None:
            ...
            return (
                self._phred_char_to_q_score_int()
            )  # Convert character to Q-score. One-step conversion
        elif self.prob_error is not None:
            return (
                self._prob_error_to_phred_q_score_int()
            )  # Also one step, since Q-score is in the middle
        else:
    ...
    def to_prob_error(self) -> float:
        if self.prob_error is not None:
            ...
            return (
                self._phred_q_score_int_to_prob_error()
            )  # Convert Q-score to probability of error
        elif self.char is not None:
            self._phred_char_to_q_score_int()  # Convert character to Q-score first
            return (
                self._phred_q_score_int_to_prob_error()
            )  # Then convert to probability of error
        else:
            raise ValueError("No Phred score information set.")
...
class PhredString:
    ...
    def __init__(
        self, phred_string: str = None, phred_chars: List[NucleotideQuality] = None
    ):
        ...
        if not any(
            ...
            raise ValueError(
                "At least one of the following must be set: phred_string, phred_chars."
            )
        if phred_chars is None:
            self.phred_chars = [NucleotideQuality(char=char) for char in phred_string]
        if phred_string is None:
            self.phred_string = "".join(
                [pos_phred.to_phred_char() for pos_phred in self.phred_chars]
            )
    ...
    def adjust_values(self, adjustment_list: List[float]):
        assert len(adjustment_list) == len(self.phred_chars), (
            "Adjustment list must be the same length as Phred string."
        )
        for i, adjustment in enumerate(adjustment_list):
            self.phred_chars[i] = NucleotideQuality(
                prob_error=clamp(
                    (self.phred_chars[i].to_prob_error() * adjustment), 0, 1
                ),
            )
...
def avg_phred_strings(phred_strings: List[PhredString]) -> PhredString:
    ...
    for nucl_pos in range(len(phred_prob_errors[0])):
        pos_prob_errors = [
            ind_phred_prob_errors[nucl_pos]
            for ind_phred_prob_errors in phred_prob_errors
        ]
        avg_prob_errors.append(sum(pos_prob_errors) / len(pos_prob_errors))
    # Now we need to convert these back to Phred scores
    avg_phred_scores = [
        NucleotideQuality(prob_error=prob_error).to_phred_char()
...
def avg_phred_char_strings(phred_chars: List[str]) -> str:
    return avg_phred_strings(
        [PhredString(phred_char) for phred_char in phred_chars]
    ).to_phred_string()
...
def show_phred_table():
    ...
    for val in range(0, 42 + 1):
        ...
        print(
            f"Q:{i.to_q_score():>3}, ASCII: {val + 33},"
...
if __name__ == "__main__":
    ...
    for i in tqdm(
        range(100),
        desc=f"Testing Speed while averaging {len(tester_phreds):>4} Phred strings",
    ):
    ...
    test_confidence_scores = [
        clamp(random.random(), 0.5, 1.0) for _ in range(len(tester_phreds[0]))
    ]
